...
import { ensureDirExists, getDataPath } from "../shared/utils.js";
import {
...
function generateYouTubeChapterDescription(
	...
	const representativeChapters =
		selectRepresentativeChapters(processedChapters);
...
export async function processTranscript(videoId: string): Promise<void> {
	try {
		...
		const youtubeChapterDescription =
			generateYouTubeChapterDescription(processedChapters);
