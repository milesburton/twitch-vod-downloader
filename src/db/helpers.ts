...
import type { Transcript, Video } from "../shared/types.js";
...
export function getVideoById(
	db: sqlite3.Database,
	id: string,
	cb: (err: Error | null, video: Video | null) => void,
) {
	db.get(`SELECT * FROM videos WHERE id = ?`, [id], (err, row) =>
		cb(err, row ? transformVideo(row) : null),
	);
}
...
export function getAllVideos(
	...
	db.all(`SELECT * FROM videos ORDER BY created_at DESC`, [], (err, rows) =>
		cb(err, rows ? rows.map(transformVideo) : []),
	);
}
...
export function getTranscriptByVideoId(
	db: sqlite3.Database,
	videoId: string,
	cb: (err: Error | null, transcript: Transcript | null) => void,
) {
	db.get(
...
export function searchTranscripts(
	db: sqlite3.Database,
	searchQuery: string,
	cb: (err: Error | null, transcripts: Transcript[]) => void,
) {
	db.all(
...
export function deleteTranscriptByVideoId(
	db: sqlite3.Database,
	videoId: string,
	cb?: (err: Error | null) => void,
) {
...
export function getVideoByIdAsync(
	db: sqlite3.Database,
	id: string,
): Promise<Video | null> {
...
export function insertVideoAsync(
	db: sqlite3.Database,
	video: Video,
): Promise<void> {
	return new Promise((resolve, reject) => {
...
export function getTranscriptByVideoIdAsync(
	db: sqlite3.Database,
	videoId: string,
...
export function insertTranscriptAsync(
	db: sqlite3.Database,
	transcript: Transcript,
): Promise<void> {
	return new Promise((resolve, reject) => {
...
export function deleteTranscriptByVideoIdAsync(
	db: sqlite3.Database,
	videoId: string,
): Promise<void> {
	return new Promise((resolve, reject) => {
...
export function deleteVideoByIdAsync(
	db: sqlite3.Database,
	videoId: string,
): Promise<void> {
	return new Promise((resolve, reject) => {
