...
import { type Video, WhisperOutput } from "../shared/types";
import {
	execWithLogs,
	execWithOutput,
	getDataPath,
...
async function cleanupFiles(directory: string, videoId: string) {
	...
	for (const entry of entries) {
		if (
			...
			await fs
				...
				.catch(() => console.warn(`Could not remove file: ${entry.name}`));
		}
	}

	const transcriptEntries = await fs.readdir(transcriptsDir, {
		withFileTypes: true,
	});
	for (const entry of transcriptEntries) {
		if (
			...
			await fs
				.rm(join(transcriptsDir, entry.name), { force: true })
				.catch(() => console.warn(`Could not remove file: ${entry.name}`));
		}
	}
}
